      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between space-y-4">
          <div>
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold text-slate-700 bg-slate-100 px-2 py-1 rounded-lg border border-slate-200">
                Passe o mouse nos eixos
              </span>
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="75%" data={radar_data}>
                  ...
                  <Radar
                    ...
                    fillOpacity={0.35}
                    strokeWidth={2.5}
                    dot={{ r: 4, fill: '#004B8D', strokeWidth: 1, stroke: '#ffffff' }}
          ...
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-700 flex items-center justify-between">
            <span>Escala: <strong>0 a 10 pontos</strong> (Pontos médios oficiais)</span>
        ...
        <div className="lg:col-span-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between space-y-4">
          <div>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-700 mt-0.5">
                  Selecione uma dimensão para ver a justificativa dos expositores
                </p>
              </div>
              <span className="text-[10px] font-bold text-fecomercio-blue bg-blue-50 border border-blue-200 px-2 py-0.5 rounded-full">
            ...
            <div className="grid grid-cols-3 gap-1.5 my-3">
              {dimensoes_operacionais.map((d, idx) => {
                ...
                return (
                  ...
                  >
                    <p className="font-bold truncate text-[11px]">{d.sigla}</p>
                    <p className={`text-[10px] font-mono font-bold mt-0.5 ${isSel ? 'text-amber-300' : 'text-slate-500'}`}>
                      Nota {d.nota.toFixed(2)}
                    </p>
                  </button>
                )
              })}
            ...
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-600">Dimensão em Foco:</span>
                  <h4 className="text-sm font-bold text-slate-900">{selectedDim.dimensao}</h4>
                </div>
                <div className="text-right">
                  <span className="text-lg font-extrabold text-slate-900">{selectedDim.nota.toFixed(2)}</span>
                  <span className="text-xs text-slate-600 block">/ 10</span>
                </div>
              </div>
              ...
              <div className="space-y-1 text-xs">
                <div className="flex justify-between text-[11px] text-slate-600">
                  <span>Distribuição de Avaliações ({selectedDim.respostas_validas} válidas):</span>
                ...
                <div className="grid grid-cols-4 gap-1.5 text-center text-[10px] font-semibold">
                  <div className="bg-emerald-100 text-emerald-800 p-1.5 rounded-lg border border-emerald-200">
                    Nota 10: <strong>{selectedDim.distribuicao.nota_10}</strong>
                  </div>
                  <div className="bg-blue-100 text-blue-800 p-1.5 rounded-lg border border-blue-200">
                  ...
                  <div className="bg-red-100 text-red-800 p-1.5 rounded-lg border border-red-200">
                    ≤ 5: <strong>{selectedDim.distribuicao.nota_ate_5}</strong>
                  </div>
                </div>
              </div>

              {/* Citação Literal dos Expositores */}
              <div className="p-3 bg-white rounded-lg border border-slate-200 space-y-1">
                <div className="flex items-center gap-1 text-[11px] font-bold text-slate-700">
                  <MessageSquareQuote className="w-3.5 h-3.5 text-fecomercio-gold" />
                  <span>Citação Representativa dos Expositores:</span>
                </div>
                <p className="text-xs text-slate-800 italic leading-relaxed">
                  "{selectedDim.citacao_destaque}"
                </p>
          ...
          <div className="text-[11px] text-slate-600 flex items-center gap-1.5">
            ...
            <span>Dados consolidados da pesquisa empírica oficial realizada pela ASA e Fecomércio AL.</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <YAxis dataKey="name" type="category" tick={{ fontSize: 10, fill: '#334155' }} width={130} />
                <Tooltip 
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            ...
            <PhoneCall className="w-4 h-4 text-fesuper-emerald" />
          </div>
