    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2 text-xs">
          <div className="flex items-center gap-1 bg-slate-50 border border-slate-200 rounded-lg p-1">
            <button
              ...
              className={`px-2.5 py-1 rounded-md font-medium transition-all ${selectedTipo === 'todos' ? 'bg-white font-bold text-slate-800 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
            >
              Todos os Públicos
            </button>
            <button
              onClick={() => setSelectedTipo('Expositor')}
              className={`px-2.5 py-1 rounded-md font-medium transition-all ${selectedTipo === 'Expositor' ? 'bg-white font-bold text-fecomercio-blue shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
            >
              Expositores (B2B)
            </button>
            <button
              onClick={() => setSelectedTipo('Visitante')}
              className={`px-2.5 py-1 rounded-md font-medium transition-all ${selectedTipo === 'Visitante' ? 'bg-white font-bold text-fesuper-emerald shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
            >
              Visitantes
            </button>
          </div>
          ...
          <div className="flex items-center gap-1 bg-slate-50 border border-slate-200 rounded-lg p-1">
            <button
              ...
              className={`px-2.5 py-1 rounded-md font-medium transition-all ${selectedCategoria === 'todos' ? 'bg-white font-bold text-slate-800 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
            >
              Todas as Categorias
            </button>
            <button
              onClick={() => setSelectedCategoria('Abaixo')}
              className={`flex items-center gap-1 px-2.5 py-1 rounded-md font-medium transition-all ${selectedCategoria === 'Abaixo' ? 'bg-white font-bold text-red-600 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1 rounded-md font-medium transition-all ${selectedCategoria === 'Sugestao' ? 'bg-white font-bold text-amber-600 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
            >
          ...
          <span className="text-slate-600 font-semibold ml-auto text-xs">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredFeedbacks.length === 0 ? (
          <div className="col-span-2 bg-white p-12 text-center rounded-2xl border border-slate-200 space-y-2">
            <p className="text-slate-500 text-sm font-semibold">Nenhum comentário encontrado com os filtros selecionados.</p>
            <button 
                ...
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between gap-2 text-xs">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-slate-700 truncate max-w-[180px] sm:max-w-[220px]" title={fb.empresa_ou_perfil}>
                    ...
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border flex items-center gap-1 ${
                      ...
                      <span>{isNegative ? 'Abaixo da Expectativa' : 'Sugestão'}</span>
                    </span>
                  </div>

                  <blockquote className="text-slate-800 text-xs sm:text-sm font-medium leading-relaxed bg-slate-50 p-3.5 rounded-xl border border-slate-200 italic">
                    "{fb.texto}"
                  </blockquote>
                </div>
                ...
                <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-100 text-xs">
                  <div className="flex flex-wrap items-center gap-1.5">
                    {fb.tags.map((tag, tIdx) => (
                      <span 
                        key={tIdx} 
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${getTagBadgeStyle(tag)}`}
                      >
                        {tag}
                      </span>
                    ))}
